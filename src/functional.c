...
int select_by_driver(sqlite3 *db, char *driver) {
    char* sql1 = malloc(120);
    strcpy(sql1, "");
    strcat(sql1, "SELECT * FROM PARK_ORDERS where last_name = '");
    //"SELECT * FROM PARK_ORDERS where last_name = '";
    strcat(sql1, driver);
    strcat(sql1, "';");
    printf("Sql in select_by_driver: %s\n", sql1
            );
   // char* sql = (char*)malloc(300);
   // snprintf(sql, 300, "%s%s%s", sql1, driver, "';");
//    free(sql);
    return select_data(db, sql1);
}
...
int select_data(sqlite3 *db, char *sql) {
    ...
    int rc = sqlite3_exec(db, sql, callback, (void *)&count, &zErrMsg);
